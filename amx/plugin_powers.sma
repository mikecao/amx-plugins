...
public powers_add(player,POWERS_FLAG)
{
	...
	if (is_user_alive(player)) {
		...
		if (POWERS_FLAG==POWERS_GOD) {
			...
			set_user_armor(player,floatround(DEFAULT_ARMOR))
		}
	...
	for (new i = 0; i < iNumPlayers; i++) {
		...
		if (iPlayerFlags) {
			...
			if (iPlayerFlags&POWERS_RAMBO) {
				set_user_armor(iPlayer,floatround(DEFAULT_ARMOR))
			}
			// Grenadier
...
{	
	...
	if (iVictimFlags) {
		...
		if (iVictimFlags&POWERS_RAMBO) {
			set_user_armor(iVictim,floatround(DEFAULT_ARMOR));
		}
	}
	return PLUGIN_CONTINUE
}
...
public plugin_init()
{
	register_plugin("Plugin Powers","1.0","mike_cao")
