...
public event_respawn(id)
{
	if (get_cvar_num("amx_hunting_mode")) {
		...
		if (get_user_team(id)==gHunterTeam) {
			...
			gPlayerIndex[id] = 0
			//set_user_rendering(id,kRenderFxGlowShell,255,255,255,kRenderTransAlpha,200)
			set_user_rendering(id,kRenderFxGlowShell,255,200,200,kRenderNormal,100)
			client_print(id,print_chat,"* You are a hunter! Go kill them bears!")
		}
		else {
			set_user_health(id,DEFAULT_HEALTH)
			gPlayerIndex[id] = 1
			client_print(id,print_chat,"* You are a bear! Rarw!")
...
public plugin_init()
{
	...
	register_cvar("amx_hunting_mode","1")
	return PLUGIN_CONTINUE
}
